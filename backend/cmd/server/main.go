...
func main() {
	...
	api := r.Group("/api")
	{
		...
		api.POST("/claude/execute-async", handler.ExecuteClaudeCommandAsync)
		api.GET("/claude/jobs", handler.GetAllJobs)
		api.GET("/claude/jobs/:id", handler.GetJob)
		api.GET("/claude/sessions/:session_id/jobs", handler.GetJobsBySession)
		api.POST("/claude/jobs/:id/cancel", handler.CancelJob)
		api.DELETE("/claude/jobs/:id", handler.DeleteJob)
